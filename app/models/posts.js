...
const posts = {
    ...
    +" where pr.salseman = u_s.userID and pr.employee = u_e.userID "
    ...
    +" where pr.employee is null and pr.salseman = u.userID "
    +" and pr.producttype = pt.typeID and pr.brand = b.brandID "
                +" and pic.picID in (select pic1.picID from picture pic1 where pic1.productID = pr.productID  limit 1)"
                +"order by pr.productID desc", function(err, res){
			if (err != null){
	...
	getPost: function(productID, callback){
		pool.query("select * from product where productID = $1::int", [productID], function(err, res){
			if (err != null){
				callback(err, null);
			}
			else {
