...
const Cart = {
	...
	getAllItems: function(customerid, callback){
		pool.query("select distinct on (p.productid) p.productid, p.name, c.quantity, p.price, picid from cart c, product p, picture pic where c.productid = p.productid and c.customerid = $1::int and isPaid = false and pic.productid = p.productid", [customerid], 
			function(err, res){
			if (err != null){
				console.log(err);
				callback(err, null);
	...
	addExistingItem: function(customerid, productid, quantity, result, callback){
		pool.query("update cart set quantity = quantity + $3::int where customerid = $1::int and productid = $2::int", [customerid, productid, quantity], 
			function(err, res){
			if (err != null){
				console.log(err);
				callback(err, null, result);
